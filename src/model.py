...
class Conv1D(nn.Module):
    ...
    def execute(self, x):
        ...
        x = self.bias + jt.matmul(x.view(-1, x.size(-1)), self.weight)
        x = x.view(*size_out)
        return x
    ...
    def execute(self, x, history=None, layer_past=None, len_past=None):
        ...
        if layer_past is not None:
            ...
            if len_past is None:
                ...
                _batch = jt.arange(0, key.shape[0], dtype=jt.int64)#, device=key.device)
    ...
    def execute(
        ...
        if position_ids is None and len_past is None:
            position_ids = jt.arange(
                ...
                dtype=jt.int64, # device=input_ids.device
            )
    ...
    def execute(
        ...
        if lm_labels is not None:
            ...
            if is_report_accuracy:
                ...
                _t1_acc = jt.zeros(_batch, dtype=jt.float32)#, device=input_ids.device)
